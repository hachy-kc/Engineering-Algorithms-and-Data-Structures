...
void merge(int arr[],int a1, int b1, int a2,int b2){
    ...
    while(a<=b1 && b<=b2){ // while the elements are in both lists
        ...
        fprintf(stderr, "               tempc = %d and arraya = %d and arrayb = %d COMPARED\n", temp[c], arr[a], arr[b]);
    }

    while(a<=b1){ //copying remaining elements of the first list
        myCopy(&arr[a++],&temp[c++]) ;
        fprintf(stderr, "tempc = %d and arraya = %d and arrayb = %d COPIED\n", temp[c], arr[a], arr[b]);
    }
    ...
    for(a=a1,c=0;a<=b2;a++,c++){//swapping the elements from temp[] to arr[]
        mySwap(&arr[a],&temp[c]);
        fprintf(stderr, "tempc = %d and arraya = %d SWAPPED\n", temp[c], arr[a]);
    }
}
